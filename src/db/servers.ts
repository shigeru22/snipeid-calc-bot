...
class DBServers {
   ...
   */
  static async getAllServers(db: Pool): Promise<IDBServerData[]> {
    const selectQuery = `
      SELECT
        servers."serverid",
        servers."discordid",
    ...
    try {
      serversDataResult = await db.query<IDBServerQueryData>(selectQuery);
    }
    catch (e) {
   ...
   * @throws { DatabaseClientError } Unhandled client error occurred.
   */
  static async insertServer(db: Pool, serverDiscordId: string): Promise<void> {
    const insertQuery = `
...

    {
      ...
      try {
        serverData = await this.getServerByDiscordId(db, serverDiscordId);
      }
      catch (e) {
   ...
   * @throws { DatabaseClientError } Unhandled client error occurred.
   */
  static async setServerCountry(db: Pool, serverDiscordId: string, countryCode: string | null): Promise<void> {
    const updateQuery = `
      UPDATE servers
   ...
   * @throws { DatabaseClientError } Unhandled client error occurred.
   */
  static async setVerifiedRoleId(db: Pool, serverDiscordId: string, verifiedRoleId: string | null): Promise<void> {
    const updateQuery = `
      UPDATE servers
   ...
   * @throws { DatabaseClientError } Unhandled client error occurred.
   */
  static async setCommandsChannelId(db: Pool, serverDiscordId: string, commandsChannelId: string | null): Promise<void> {
    const updateQuery = `
      UPDATE servers
   ...
   * @throws { DatabaseClientError } Unhandled client error occurred.
   */
  static async setLeaderboardsChannelId(db: Pool, serverDiscordId: string, leaderboardsChannelId: string | null): Promise<void> {
    const updateQuery = `
      UPDATE servers
   ...
   * @throws { DatabaseClientError } Unhandled client error occurred.
   */
  static async isCommandChannel(db: Pool, serverDiscordId: string, channelId: string): Promise<boolean> {
    try {
